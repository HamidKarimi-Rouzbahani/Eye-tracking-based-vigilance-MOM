...
%             end
            ...
            for dot_num=1:Num_moving_dots*Number_of_trials_in_blocks
                ...
                if total_counter>appearance_time(dot_num)
                    ...
                    if sum(dot_num_closest_top2==top_events(:,tr))==1 && isnan(manually_deflected_top(dot_num_closest_top,tr)) && isnan(beeped_top(dot_num_closest_top,tr)) && keycode(KbName(response_button)) && refractory_counter>refractory_period && closest_top_is_left(1,tr)==1 && (counter1((tr-1).*Num_moving_dots+dot_num_closest_top)-obstacle_xposition1)<0 && counter1((tr-1).*Num_moving_dots+dot_num_closest_top)>x_boundary %&& length(responseIdx)==1
...
%                     elseif length(responseIdx)==1 && (refractory_counter_beep_off>refractory_period_beep_off)
%                         beepIncorrect;
%                         refractory_counter_beep_off=0;
                    end
