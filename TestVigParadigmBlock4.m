function TestVigParadigmBlock4(Subj)
...
try
    ...
    for Block_Num=1:Number_of_blocks
        ...
        dot_color=zeros(1,Num_moving_dots*Number_of_trials_in_blocks);
        dot_color(randsample([1:length(dot_color)],length(dot_color)./2))=1;
        dot_color2=zeros(1,Num_moving_dots*Number_of_trials_in_blocks);
        dot_color2(randsample([1:length(dot_color2)],length(dot_color2)./2))=1;
...
%                 tp(iter,:)=randsample([(tr-1)*trial_time+1+distance_between_dots_limit:distance_between_dots_limit:tr*trial_time-distance_between_dots_limit],sum(appearance_time_temp(:,tr)==0));
        ...
        %         Eyelink('command', 'record_status_message "BLOCK %d/%d, Task %s"', Block_Num, Number_of_blocks, Task);
        ...
        for total_counter=1:trial_time*(Number_of_trials_in_blocks+0.5)
